...
function Header({ session }: { session: workshopType }) {
  return (
    <DialogHeader>
      <Image src={session.logo ?? "/default-logo.png"} alt={session.title} width={100} height={100} className="mx-auto w-7/10" />
      <DialogTitle />
      <div className="flex items-center gap-4">
        <Image src={session.instructor.photo ?? "/default-logo.png"} alt={session.title} width={100} height={100} className="size-10 rounded-full" />
